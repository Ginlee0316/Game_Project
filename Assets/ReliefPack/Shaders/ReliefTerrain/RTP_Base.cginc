...
//	return;
	...
	o.distance=_INPUT_distance; // needed for fog
    	...
    	float3 worldNormalFlat;
    	{
			...
			// DON'T remove underlying double space (for underlying geom blend we don't have tessellation but we still need right world normal vector) !!!
			float3  worldNormalFlat=WorldNormalVector(IN, float3(0,0,1)); // WORLD norm w/o bumpmaps	
		#else
			// below line doesn't work in pass generated by Unity using addshadow in surface shader, so LOD manager is supposed to comment WorldNormalVector() call even if it's not really used anywhere)
			// HACK - LOD manager will commentout below world normal function call... (don't touch this line without conforming this with LOD manager handling !!!)
			float3 worldNormalFlat=WorldNormalVector(IN, float3(0,0,1)); // WORLD norm w/o bumpmaps	
		#endif
	#endif
...
//			#endif
		...
		if (_uv_Relief_w<1) {
		 	...
		 	fixed4 col;
				...
				col.rgb = global_color_value.rgb*dot(splat_control, tex2Dd(_SSColorCombinedA, _INPUT_uv.xy, _ddxMain.xy, _ddyMain.xy));
		 	#else
				col = splat_control.r * tex2Dd(_SplatA0, _INPUT_uv.xy, _ddxMain.xy, _ddyMain.xy);
				col += splat_control.g * tex2Dd(_SplatA1, _INPUT_uv.xy, _ddxMain.xy, _ddyMain.xy);
				col += splat_control.b * tex2Dd(_SplatA2, _INPUT_uv.xy, _ddxMain.xy, _ddyMain.xy);
				col += splat_control.a * tex2Dd(_SplatA3, _INPUT_uv.xy, _ddxMain.xy, _ddyMain.xy);
			...
			o.Specular=lerp(o.Specular, 0.5, _uv_Relief_w);
				...
				normals_combined = tex2Dd(_BumpMap01, _INPUT_uv.xy, _ddxMain.xy, _ddyMain.xy).rgba*splat_control.rrgg;
				normals_combined+=tex2Dd(_BumpMap23, _INPUT_uv.xy, _ddxMain.xy, _ddyMain.xy).rgba*splat_control.bbaa;
	...
	IN.viewDir.z=saturate(IN.viewDir.z); // czasem wystepuja problemy na krawedziach widocznosci (viewDir.z nie powinien byc jednak ujemny)
		...
		#ifdef LOCAL_SPACE_UV
			float3 worldNormalFlatTMP=mul(unity_WorldToObject, float4(worldNormalFlat,0)).xyz;
		#else
				...
				for(int i=0; i<_TERRAIN_SHADOW_STEPS; i++) {
					...
					if (hit_flag) {
						...
						for(int i=0; i<_TERRAIN_SHADOW_SMOOTH_STEPS; i++) {
							rayPos_shadows.xyz+=EyeDirTan;
							float d=dot(splat_control1, tex2Dlod(_TERRAIN_HeightMap, rayPos_shadows.xyww))+dot(splat_control2, tex2Dlod(_TERRAIN_HeightMap2, rayPos_shadows.xyww)) - rayPos_shadows.z;
			...
			float4 gloss=0;
				...
				if (triplanar_blend_flag) {
					...
					if (triplanar_blend.x>0.02) {
						...
						normals_combined = tex2Dd(_BumpMap01, uvTRI1.xy, _ddxMain.yz, _ddyMain.yz).rgba*splat_control1.rrgg;
						#if !defined(USE_2_LAYERS_ONLY)
						normals_combined+=tex2Dd(_BumpMap23, uvTRI1.xy, _ddxMain.yz, _ddyMain.yz).rgba*splat_control1.bbaa;
						#endif							
					...
					if (triplanar_blend.y>0.02) {
						...
						normals_combined = tex2Dd(_BumpMap01, uvTRI2.xy, _ddxMain.xy, _ddyMain.xy).rgba*splat_control1.rrgg;
						#if !defined(USE_2_LAYERS_ONLY)
						normals_combined+=tex2Dd(_BumpMap23, uvTRI2.xy, _ddxMain.xy, _ddyMain.xy).rgba*splat_control1.bbaa;
						#endif							
					...
					if (triplanar_blend.z>0.02) {
						...
						normals_combined = tex2Dd(_BumpMap01, uvTRI3.xy, _ddxMain.xz, _ddyMain.xz).rgba*splat_control1.rrgg;
						#if !defined(USE_2_LAYERS_ONLY)
						normals_combined+=tex2Dd(_BumpMap23, uvTRI3.xy, _ddxMain.xz, _ddyMain.xz).rgba*splat_control1.bbaa;
						#endif							
					...
					float4 normA=tex2Dd(_BumpMap01, uvTRI_tmp.xy, _ddxMainTRI.xy, _ddyMainTRI.xy).rgba;
					float4 normals_combined = normA*splat_control1.rrgg;
					#if !defined(USE_2_LAYERS_ONLY)
					float4 normB=tex2Dd(_BumpMap23, uvTRI_tmp.xy, _ddxMainTRI.xy, _ddyMainTRI.xy).rgba;
		 	...
		 	//
				...
				for(int i=0; i<_TERRAIN_SHADOW_STEPS; i++) {
					...
					if (hit_flag) {
						...
						for(int i=0; i<_TERRAIN_SHADOW_SMOOTH_STEPS; i++) {
							rayPos_shadows.xyz+=EyeDirTan;
							float d=dot(splat_control1, tex2Dlod(_TERRAIN_HeightMap, rayPos_shadows.xyww)) - rayPos_shadows.z;
							smooth_val+=saturate(d);
							if (smooth_val>break_val) break;
						}
						shadow_atten=saturate(1-smooth_val/break_val);
