...
//
Shader "Relief Pack/GeometryBlend_WaterShader_FlowMap_HB" {
    Properties {
	...
	ZTest LEqual		
	CGPROGRAM
	#pragma surface surf BlinnPhong vertex:vert decal:blend
	...
	void surf (Input IN, inout SurfaceOutput o) {
		...
		p*=p;
      	...
      	//fixed4 col = tex2D (_MainTex, rayPos.xy);
      	fixed4 col = tex2D (_MainTex, IN.custUV.xy);
        o.Gloss = col.a;
        float GlossDry=o.Gloss;
		...
		col.rgb*=1-saturate(TERRAIN_LayerWetStrength*2)*0.3;
		#else
        o.Normal = UnpackNormal (tex2D (_BumpMap, IN.custUV.xy));
        #endif
		...
		norm_far.xy = global_bump_val.rg*2-1;
		norm_far.z = sqrt(1 - saturate(dot(norm_far.xy, norm_far.xy)));      	
		norm_far=lerp( float3(0,0,1), norm_far, _uv_Relief_w*_BumpMapPerlinBlend );	
