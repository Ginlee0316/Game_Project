...
Shader "Hidden/TenkokuSunShafts" {
	...
	half4 fragScreen(v2f i) : SV_Target { 
		...
		#endif
		half4 depthMask = saturate (colorB * lerp(_SunColor*_TintColor,_TintColor,_TintColor.a));
		//depthMask = saturate(lerp(-1,1.0,depthMask));
	...
	half TransformColor (half4 skyboxValue) {
		return dot(max(skyboxValue.rgb - _ColorBlock.rgb, half3(0,0,0)), half3(1,1,1)); // threshold and convert to greyscale
	}
